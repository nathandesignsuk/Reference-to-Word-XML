...
def convert_reference():
    if request.method == 'POST':
        ...
        with open(tmp_path, 'wb') as f:
            f.write(xml_output)
        return send_file(tmp_path, as_attachment=True, download_name="word_bibliography.xml")
...
def create_word_xml(entries):
    ...
    for entry in entries:
        tag = entry.get("ID", entry.get("title", "Reference"))[:15]
...
def parse_mendeley_xml(xml_bytes):
    ...
    for item in root.findall('.//record'):
        entry = {
            'title': item.findtext('titles/title', default='Untitled'),
